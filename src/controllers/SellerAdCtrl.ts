...
// ==========================================
// 📌 دریافت همه آگهی‌ها (عمومی، بدون محدودیت کاربر)
// ==========================================
...
// ==========================================
// 📌 دریافت آگهی‌های یک کاربر خاص با فیلترهای پیشرفته (جدید)
// ==========================================
export const getSellerAdsByOwnerWithFilters = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    // ---------- ساخت where پویا (دقیقاً مانند getAllSellerAds + شرط owner) ----------
    const where: any = {
      owner: ownerId,
      adStatus: "approved", // فقط آگهی‌های تأییدشده
    };
    ...
    console.error("❌ ERROR GETTING SELLER ADS BY OWNER WITH FILTERS:", err);
    res.status(500).json({ error: err.message });
  }
};
...
// ==========================================
// 📌 گرفتن آگهی‌های یک کاربر خاص (بدون فیلترهای پیشرفته)
// ==========================================
